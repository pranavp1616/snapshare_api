...
def getData(request,posts):
    ...
    for i in posts:
        ...
        final_data.append({
                'id':i.id,
                'image':i.image.url, # httphdr+request.get_host()+i.image.url - set this if its not working in localhost -(to return full URL so that UI running on other domains can display image)
